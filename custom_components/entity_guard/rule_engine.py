...
class RuleEngine:
    ...
    async def async_evaluate(self, entity_id: str, new_state: Any) -> None:
        ...
        triggered = self._trigger_decision(entity_id, new_state)
    ...
    def _color_trigger_decision(self, new_state: Any) -> bool:
        ...
        Skips only genuinely unreachable states (unavailable/unknown); an off
        light is NOT skipped — light.turn_on with the target color will turn it
        on and set the color in one call.
        """
        attr = self._config.attribute
        if attr is None:
            return False
    ...
    def _schedule_delayed_enforcement(self, entity_id: str) -> None:
        ...
        async def _fire(_now: datetime) -> None:
            ...
            if not self._state.enabled or not self._master_enabled_getter():
                ...
                return
            current = self._hass.states.get(entity_id)
            triggered = self._trigger_decision(entity_id, current)
    ...
    async def async_clear_history(self) -> None:
        ...
        # _persist() serializes the full canonical runtime blob (already carries
        # the cleared counters set above). Flush synchronously so the reset —
        # including counter_total_since — survives an immediate HA crash.
        self._persist()
        await self._store.async_save_now()
        if self._current_status == STATUS_ERROR:
