...
async def test_color_enforcement_calls_light_turn_on_with_rgb(hass: HomeAssistant):
    ...
    target_str = enforced_events[-1].data["target"]
    assert "rgb_color" in target_str and "255" in target_str
...
async def test_color_enforcement_turns_on_off_light(hass: HomeAssistant):
    """An off light with the wrong color triggers light.turn_on with the target color."""
    config = _make_config(
        mode=MODE_ATTRIBUTE,
        attribute=ATTR_RGB_COLOR,
        target_value=[255, 0, 0],
...
async def test_delayed_color_enforcement_fires_when_light_turns_off(hass: HomeAssistant):
    """Delayed enforcement fires even if the light turns off before the timer; light.turn_on sets color."""
...
async def test_manual_color_enforce_turns_on_off_light(hass: HomeAssistant):
    """Manual test-enforce on an off light calls light.turn_on with the target color."""
    config = _make_config(
        mode=MODE_ATTRIBUTE,
        attribute=ATTR_RGB_COLOR,
        target_value=[255, 0, 0],
